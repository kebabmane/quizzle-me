class Admin::FactsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @fact.save
        format.html { redirect_to admin_fact_path(@fact), notice: 'Fact was successfully created.' }
  ...
  def update
    ...
    format.html { redirect_to admin_fact_path(@fact), notice: 'fact status updated' }
    format.js   { redirect_to admin_fact_path(@fact), notice: 'fact status updated' }
  end
